...
# Fill in User/Pass, all the Mount Servers you want to copy the rules to and yes/no for a small/full set of Yara Rules
$Username = "BACKUP\Administrator"
$Password = "Veeam123!"
$mountservers = "VBR.BACKUP.LAB","REPO1-1.BACKUP.LAB","PROXY1-1.BACKUP.LAB"
$lab = "yes"
...
Expand-Archive -LiteralPath '.\YaraRules.zip' -DestinationPath .\
...
foreach ($Session in $Sessions) { Invoke-Command -Session $Session {(Compress-Archive -Path "C:\Program Files\Veeam\Backup and Replication\Backup\YaraRules" -DestinationPath "C:\Program Files\Veeam\Backup and Replication\Backup\YaraRules_Backup-$(get-date -f dd-MM-yyyy_HHmm).zip" )} } 
